...
(async()=>{const b=await chromium.launch({channel:'msedge',headless:true});const errors=[],results=[];try{
 ...
 for(const [width,height] of [[1920,1080],[1536,864],[1366,768],[1280,720],[430,932],[390,844],[360,800]]){
  await p.setViewportSize({width,height});await p.goto(process.env.SOLO_URL||'http://127.0.0.1:8080/jungle.html');await p.screenshot({path:path.join(out,`solo-landing-${width}.png`),fullPage:true});assert.ok(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),'landing overflow '+width);
  ...
  await p.locator('#solo-share').waitFor();assert.ok(await p.evaluate(()=>JungleSolo.result&&Number.isFinite(JungleSolo.result.score)),'solo result');await p.screenshot({path:path.join(out,`solo-result-${width}.png`),fullPage:true});await p.locator('#solo-share').click();const challenge=await p.evaluate(()=>JungleSolo.result.seed);await p.goto((process.env.SOLO_URL||'http://127.0.0.1:8080/jungle.html')+'?challenge='+challenge);assert.deepEqual(await p.evaluate(()=>JungleSolo.state.scenarios.map(s=>s.id)),seed,'shared deterministic sequence');
  await p.goto(process.env.HUB_URL||'http://127.0.0.1:8080');await p.waitForFunction(()=>window.GameManager&&window.Rooms);await p.waitForTimeout(2300);
