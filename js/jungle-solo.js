...
function start(s,kind){
 ...
 $('solo-landing').hidden=true;$('solo-session').hidden=false;$('solo-mode').textContent=kind==='daily'?'DESAFIO DO DIA':kind==='challenge'?'DESAFIO COMPARTILHADO':'SOLO TRAINING';paint();window.scrollTo(0,0);
}
...
function finish(standings){
 ...
 $('game-root').innerHTML='<div class="solo-results"><p class="solo-eyebrow">TREINO CONCLUÍDO</p><h1>'+greats+' / 5 BEST CALLS</h1><div class="solo-final-score">'+score+' <small>/ 5000</small></div><p>'+ (greats>=4?'Boa leitura de mapa. Agora teste outra sequência.':greats>=2?'Você encontrou boas janelas. Revise o custo das jogadas arriscadas.':'Cada erro mostrou uma pista. Tente de novo e observe a prioridade das lanes.')+'</p><div class="solo-result-actions"><button id="solo-retry" class="btn-arcade primary">NOVO TREINO</button><button id="solo-share" class="btn-arcade ghost">COMPARTILHAR DESAFIO</button></div><div id="solo-share-text"></div></div>';$('solo-controls').innerHTML='';$('solo-retry').onclick=()=>start(randomSeed(),'training');$('solo-share').onclick=share;
}
...
function home(){run++;state=null;g.cleanup();$('solo-session').hidden=true;$('solo-landing').hidden=false;$('solo-controls').innerHTML='';window.scrollTo(0,0);}
...
$('solo-start').onclick=()=>start(randomSeed(),'training');$('solo-daily').onclick=()=>start(daySeed(),'daily');$('solo-exit').onclick=home;
